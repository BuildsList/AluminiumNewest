...
/*
world
	hub = "Garthor.DungeonGenerator"
	loop_checks = 0
	mob = /mob/Player
	maxx = 100
	maxy = 100
	view = 10
	turf = /turf/simulated/wall
	New()
		..()
		spawn(1) mapGen()
*/
proc
	mapGen()
		var/turf/simulated/wall/D = locate(rand(6,world.maxx-5),rand(6,world.maxy-5),1)
...
structure
		...
		New(var/turf/loc,var/dir,var/length,var/prob)
			...
			if(poss.len)
				...
				if(istype(loc,/turf/simulated/wall))
					...
					new /obj/machinery/door/unpowered/shuttle/dungeon/(loc)
				if(prob > -1)
	...
	rectroom
		New(var/turf/A,var/dim1,var/dim2,var/prob)
			...
			new /obj/machinery/door/unpowered/shuttle/dungeon/(possDoors[Z])
