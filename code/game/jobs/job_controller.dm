...
/datum/controller/occupations
	...
	proc/GiveRandomJob(var/mob/new_player/player)
		...
		for(var/datum/job/job in shuffle(occupations))
			...
			if(istype(job, GetJob("Savage"))) // We don't want to give him assistant, that's boring!
				continue
		...
		if(H.mind)
			...
			switch(rank)
				if("Cyborg")
					H.Robotize()
					return 1
				if("Observer","Chef")	//need bag preference stuff!
