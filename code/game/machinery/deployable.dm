...
/obj/structure/barricade/metal
	name = "hedgehog"
	desc = "This space is blocked off by a hedgehog."
	...
	ex_act(severity)
		switch(severity)
			if(1.0)
				visible_message("\red <B>The hedgehog is blown apart!</B>")
				del(src)
				return
			if(2.0)
				src.health -= 25
	...
	meteorhit()
		visible_message("\red <B>The hedgehog is smashed apart!</B>")
		new /obj/item/stack/sheet/metal(get_turf(src))
		new /obj/item/stack/sheet/metal(get_turf(src))
		new /obj/item/stack/rods(get_turf(src))
		del(src)
	...
	blob_act()
		src.health -= 25
		if (src.health <= 0)
			visible_message("\red <B>The blob eats through the hedgehog!</B>")
			del(src)
		return
