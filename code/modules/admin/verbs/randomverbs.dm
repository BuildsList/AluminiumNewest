...
/client/proc/toggle_map_dir()
	set category = "Special Verbs"
	set name = "Rotate Map"
	set desc = "rotates visible map"

	if(dir == NORTH)
		dir = input("Select Direction", "FUCK YE", 7) in list(1,2,4,8)
	else
		dir = NORTH

	log_admin("[key_name(usr)] changed their map rotation to  [dir].")
	//message_admins("\blue [key_name_admin(usr)] changed their view range to [view].", 1)	//why? removed by order of XSI
