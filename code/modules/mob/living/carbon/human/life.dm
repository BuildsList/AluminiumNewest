...
/mob/living/carbon/human
	...
	proc/handle_disabilities()
		...
		if (getBrainLoss() >= 60 && stat != 2)
			if (prob(3))
				if(config.use_russian)
					switch(pick(1,2,3))
						if(1)
							say(pick("Заебали, я в апатию", "Как руки минять?", ";КАПИТАН - ГОНДОН", "тритар убивает в [pick("баре","прибытии","медбее","бриге")] помогите!!!", "дайте мне [pick("телекинез","халка","эпилепсию")]?", "нонРП!!!", "хватит грифить меня!!!!", "Отвали!", "Перестань!"))
						if(2)
							say(pick("ФУС РО ДА","Ебаные фуриёбы!", "СПАРТАК - ПЕЧИПЁН!", "FOR TEH EMPRAH", "Няш-мяш", "ДВАРФЫ", "SPESS MAHREENS", "Ебаные Дворфы!", "ГРАФОООН", "потому что я могу!"))
						if(3)
							emote("drool")
				else
					switch(pick(1,2,3))
						...
						if(2)
							say(pick("FUS RO DAH","fucking 4rries!", "stat me", ">my face", "roll it easy!", "waaaaaagh!!!", "red wonz go fasta", "FOR TEH EMPRAH", "lol2cat", "dem dwarfs man, dem dwarfs", "SPESS MAHREENS", "hwee did eet fhor khayosss", "lifelike texture", "luv can bloooom", "PACKETS!!!"))
						if(3)
							emote("drool")
...
//					log_debug("[src] : Exhaling some viruses")
	...
	proc/handle_breath(datum/gas_mixture/breath)
		if(status_flags & GODMODE)
			return
		var/datum/organ/internal/lungs/lungs = internal_organs["lungs"]
		if(!breath || (breath.total_moles() == 0) || suiciding)
			...
			if(health > config.health_threshold_crit && !zombie)
				...
				failed_last_breath = 1
			if(lungs.is_bruised() > config.health_threshold_crit)
				adjustOxyLoss(HUMAN_MAX_OXYLOSS)
				failed_last_breath = 1
			else
